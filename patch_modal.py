with open(r'D:\AI\projects\mtms-aqua-haier-kb\site\js\produk.js', 'r', encoding='utf-8') as f:
    content = f.read()

old = '''      function openModal(p) {
        var rows = [
          ["Kategori", p.kategori],
...
if old in content:
    ...
    with open(r'D:\AI\projects\mtms-aqua-haier-kb\site\js\produk.js', 'w', encoding='utf-8') as f:
        f.write(content)
    print('SUCCESS: Replaced openModal function')
else:
    print('FAILED: old string not found')
    # Try to find the difference
    idx = content.find('function openModal(p)')
    if idx >= 0:
